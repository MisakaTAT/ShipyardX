...
describe('serverFormSchema', () => {
  it('rejects whitespace-only required fields', () => {
    ...
    expect(result.error?.issues.map((issue) => issue.message)).toEqual(
      expect.arrayContaining(['ui.validation.serverName', 'ui.validation.host', 'ui.validation.username'])
    )
  })
})
...
describe('serverTestConnectionSchema', () => {
  it('requires password when password auth is selected', () => {
    ...
    expect(result.success).toBe(false)
    expect(result.error?.issues[0]?.message).toBe('ui.validation.password')
  })
})
