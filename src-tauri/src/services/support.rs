    ...
    F: Future<Output = ()> + Send + 'static,
{
    ...
    register_stream_handle(state, stream_id.clone(), stop_tx, code, message)?;
    Ok(stream_id)
}

pub(crate) fn stop_managed_stream(
    state: &State<'_, AppState>,
    stream_id: &str,
    code: &'static str,
    message: &'static str,
) -> AppResult<bool> {
    if let Some(handle) = remove_stream_handle(state, stream_id, code, message)? {
    ...
    F: Future<Output = ()> + Send + 'static,
{
    ...
    spawn_on_runtime(task)?;
    register_event_stream_handle(state, server_id, handle, code, message)?;
    Ok(stream_id)
}
...
pub(crate) fn stop_managed_event_stream(
    ...
    remove_event_stream_handle(state, server_id, code, message)
}
